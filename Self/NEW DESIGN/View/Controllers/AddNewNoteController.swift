...
//  Self
//
//  Created by Thenny Chhorn on 1/22/21.
//
...
class AddNewNoteController: UIViewController {
    ...
    private lazy var navigationView = CustomNavigationView(titleString: "Write Note",
                                                           image: .saveImage!,
                                                           delegate: self)
    ...
    fileprivate func configureUI() {
     ...
     fileprivate func addNewNote() {

        guard let text = textString else {return}
        Service.handleSaveNoteData(contentName: text)
...
extension AddNewNoteController: CustomNavigationViewDelegate {
...
extension AddNewNoteController: AddContentViewDelegate {
